...
{
    auto result = analyze_text(R"(Lorem ipsum color sit amet, consectetur adipiscing elit,
                                                            sed do eiusmod tempor incididunt ut lavore et dolore magna alique.)");
    for(auto const& kvp: result)
    {
        std::cout << kvp.first << " : " << std::fixed << std::setw(5) << std::setfill(' ') << std::setprecision(2) << kvp.second << std::endl;
    }
}