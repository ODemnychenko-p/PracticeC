//
// Created by Oleksandr Demnychenko on 28.12.2024.
//
...
public:
    ...
    explicit observable_vector(size_type count, const T& value,
                               const Allocator& alloc = Allocator()):
                               data(count, value, alloc){}
    explicit observable_vector(size_type count, const Allocator& alloc = Allocator()):
            data(count, alloc){}
    explicit observable_vector(observable_vector&& other) noexcept:
            data(other.data){}
    explicit observable_vector(observable_vector&& other, const Allocator& alloc) noexcept:
            data(other.data, alloc){}
    explicit observable_vector(std::initializer_list<T> init, const Allocator& alloc = Allocator()) noexcept:
            data(init, alloc){}
    ...
    void push_back(T&& value)
    {
        ...
        for(auto o: observers)
        {
            ...
            {
                o->collection_changed({
                    collection_action::add, std::vector<size_t>
                                {
                                    data.size() - 1
                                }
                });
