...
class departament_manager_role: public role{
public:
    virtual double approval_limit() const noexcept override
    {
        return 100000;
...
public:
    ...
    void approve(expense const& e)
    {
        ...
        {
            std::cout << name << " approved expense '" << e.description << "', cost = "
            << e.amount << std::endl;
        }
...
static void task_number_4()
{
    ...
    auto david = std::make_shared<employee>("david jones",
                                           std::make_unique<departament_manager_role>());
